...
class _RunnerManagerState:
    ...
    initializing = namedtuple("initializing",
                              ["test_type", "test", "test_group", "group_metadata", "failure_count"])
    running = namedtuple("running", ["test_type", "test", "test_group", "group_metadata"])
    restarting = namedtuple("restarting", ["test_type", "test", "test_group", "group_metadata", "force_stop"])
...
class TestRunnerManager(threading.Thread):
    def __init__(self, suite_name, index, test_queue, test_source_cls,
                 browser_cls_by_test_type, browser_kwargs_by_test_type,
                 executor_cls_by_test_type, executor_kwargs_by_test_type, stop_flag, rerun=1,
        ...
        * Take any remedial action required e.g. restart crashed or hung
        ...
        self.test_type = None
        self.browser_cls_by_test_type = browser_cls_by_test_type
        self.browser_kwargs_by_test_type = {}
        for test_type, kwargs in browser_kwargs_by_test_type.items():
            if kwargs.get("device_serial"):
                kwargs = kwargs.copy()
                # Assign Android device to runner according to current manager index
                kwargs["device_serial"] = kwargs["device_serial"][index]
            self.browser_kwargs_by_test_type[test_type] = kwargs
        self.executor_cls_by_test_type = executor_cls_by_test_type
        self.executor_kwargs_by_test_type = executor_kwargs_by_test_type
    ...
    def init(self):
        ...
        if self.state.test_type != self.test_type:
            if self.browser is not None:
                self.browser.browser.cleanup()
            browser_cls = self.browser_cls_by_test_type[self.state.test_type]
            browser_kwargs = self.browser_kwargs_by_test_type[self.state.test_type]
    ...
    def start_test_runner(self):
        ...
        self.executor_cls = self.executor_cls_by_test_type[self.state.test_type]
        self.executor_kwargs = self.executor_kwargs_by_test_type[self.state.test_type]
    ...
    def after_test_end(self, test, restart, force_rerun=False, force_stop=False):
        ...
        if not force_rerun and self.run_count >= self.rerun:
            ...
            if test_type != self.state.test_type:
                self.logger.info("Restarting browser for new test type:%s" % test_type)
                restart = True
        ...
        if restart:
            return RunnerManagerState.restarting(test_type, test, test_group, group_metadata, force_stop)
        else:
            return RunnerManagerState.running(test_type, test, test_group, group_metadata)

    def restart_runner(self):
        """Stop and restart the TestRunner"""
        assert isinstance(self.state, RunnerManagerState.restarting)
        self.stop_runner(force=self.state.force_stop)
        return RunnerManagerState.initializing(self.state.test_type, self.state.test, self.state.test_group, self.state.group_metadata, 0)
...
class ManagerGroup:
    ...
    def __init__(self, suite_name, size, test_source_cls, test_source_kwargs,
                 browser_cls_by_test_type, browser_kwargs_by_test_type,
                 executor_cls_by_test_type, executor_kwargs_by_test_type,
                 rerun=1,
        ...
        self.browser_cls_by_test_type = browser_cls_by_test_type
        self.browser_kwargs_by_test_type = browser_kwargs_by_test_type
        self.executor_cls_by_test_type = executor_cls_by_test_type
        self.executor_kwargs_by_test_type = executor_kwargs_by_test_type
        self.pause_after_test = pause_after_test
        self.pause_on_unexpected = pause_on_unexpected
    ...
    def run(self, tests):
        ...
        for idx in range(self.size):
            manager = TestRunnerManager(self.suite_name,
                                        ...
                                        self.browser_cls_by_test_type,
                                        self.browser_kwargs_by_test_type,
                                        self.executor_cls_by_test_type,
                                        self.executor_kwargs_by_test_type,
                                        self.stop_flag,
                                        self.rerun,
