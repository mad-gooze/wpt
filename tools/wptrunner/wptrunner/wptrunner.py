...
def run_test_iteration(test_status, test_loader, test_source_kwargs, test_source_cls, run_info,
    ...
    for test_type in test_loader.test_types:
        tests[test_type].extend(test_loader.tests[test_type])
    ...
    executor_cls_by_test_type = {}
    executor_kwargs_by_test_type = {}
    browser_cls_by_test_type = {}
    browser_kwargs_by_test_type = {}
    ...
    for test_type in run_test_kwargs["test_types"]:
        ...
        if executor_cls is None:
            ...
            continue
        executor_cls_by_test_type[test_type] = executor_cls
        executor_kwargs = product.get_executor_kwargs(logger,
                                                      test_type,
                                                      test_environment,
                                                      run_info,
                                                      **run_test_kwargs)
        executor_kwargs_by_test_type[test_type] = executor_kwargs
        browser_cls = product.get_browser_cls(test_type)
        browser_cls_by_test_type[test_type] = browser_cls
        browser_kwargs = product.get_browser_kwargs(logger,
                                                    ...
                                                    **run_test_kwargs)
        browser_kwargs_by_test_type[test_type] = browser_kwargs

    tests_to_run = {}
    for test_type in run_test_kwargs["test_types"]:
        if test_type not in executor_cls_by_test_type:
            continue
        executor_cls = executor_cls_by_test_type[test_type]
